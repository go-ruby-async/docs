...
`go-ruby-async/async` reimplements Ruby's `async` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-async/async` directly, with no Ruby runtime;
...
Because `async` is pure Go with cgo disabled, it also compiles to **WebAssembly**
